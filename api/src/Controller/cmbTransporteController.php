...
class cmbTransporteController extends AppController {
    ...
    try{
        ...
        $sQuery = "SELECT * FROM cuotas_transportes
                WHERE estatus=1 AND 
                id_ciclo_escolar=".$IdCiclo."  AND 
        ...
        if (count($aDatosAlum)>0){
            return $this->asJson(array(
             ...
             return $this->asJson(array(
                "success" => true,
                "message" => "No hay Datos Transporte",
                "records" => $aDatosAlum,
                "metadata" => array(
                    "total_registros" => count($aDatosAlum)
                )

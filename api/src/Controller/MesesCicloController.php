...
class MesesCicloController extends AppController {
    ...
    try{
       ...
       // $intIdNivel = $this->request->query('id_nivel_educativo');
        ...
        $sQuery = "SELECT * FROM meses_ciclo where status=1";
        ...
        if (count($aDatosAlum)>0){
            return $this->asJson(array(
             ...
             return $this->asJson(array(
                "success" => true,
                "message" => "No hay Datos Meses Ciclos",
                "records" => $aDatosAlum,
                "metadata" => array(
                    "total_registros" => count($aDatosAlum)
                )

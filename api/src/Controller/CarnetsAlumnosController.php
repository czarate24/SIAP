...
class CarnetsAlumnosController extends AppController {
    ...
    try{
        ...
        $sQuery = "SELECT alum.id_alumno,
                    ...
                    ccole.id_cuota_colegiatura, 
                    UPPER(nivel.nombre) as nivel_educativo,
                    carncole.id_carnet_colegiatura,
                    carncole.id_ciclo_escolar as id_ciclo_escolar_colegiatura,
                    ...
                    ON cins.id_institucion=alum.id_institucion
                    LEFT JOIN cxcandes.cuotas_colegiaturas AS ccole
                    ON ccole.id_nivel_educativo=cins.id_nivel_educativo AND ccole.grado=cins.grado
                    LEFT JOIN sistema_andes.niveles_educativos as nivel
                    ON alum.id_nivel_educativo=nivel.id_nivel_educativo
                    LEFT JOIN cxcandes.carnets_inscripcion AS carn
                    ON  carn.id_alumno=alum.id_alumno AND (carn.id_ciclo_escolar=".$intIdCiclo.")
                    LEFT JOIN cxcandes.carnets_colegiatura AS carncole
                    ON  carncole.id_alumno=alum.id_alumno AND carncole.id_ciclo_escolar=".$intIdCiclo."
                    WHERE (CASE WHEN cins.grado<>0  OR cins.grado='MAT' THEN cins.grado=alum.grado ELSE cins.grado=0 END) 
                    AND alum.id_status_alumno=1 AND alum.id_institucion=".$intIdIns." GROUP BY alum.id_alumno";
